...
/// the **existing** window instead of letting `WindowGroup` open a second one.
///
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// AppKit delivers `shlinkly://` opens here. The `WindowGroup` is told not to
    /// handle external events, so it won't spawn a window; we park the parsed link
    /// on the shared model — the open window's navigation shell observes it and
    /// selects the link — and bring that window forward.
