...
public final class ShortURLListStore {
    ...
    public func loadFirstPage() {
        ...
        loadMoreTask?.cancel()
        loadTask?.cancel()
        // Full-screen skeleton only on a genuine first load (nothing to show yet).
        // With rows already on screen — a sort/search/filter re-query — keep them
        // visible so the new result diffs in (animated in `apply(firstPage:)`)
        // rather than blanking the list to a spinner mid-use; a sort change then
        // reads as the rows gliding to their new order.
    ...
    private func runFirstPage() async {
        do {
            ...
            // Only surface the full-screen error when there's nothing to fall back
            // to. A failed sort/search/filter re-query (rows still on screen)
            // keeps them visible — mirroring `refresh()` — instead of blanking.
            if items.isEmpty {
